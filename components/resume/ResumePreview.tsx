...
const ResumePreview: React.FC = () => {
    ...
    const styles = templateLook[selectedTemplate as keyof typeof templateLook] || templateLook.classic;
    const dynamicStyles = useDynamicStyles(selectedTemplate);
    ...
    const renderSectionHeader = (title: string) => {
        ...
        const iconElement = (selectedTemplate === 'minimalist' || styles.layout === 'two') ? React.cloneElement(icons[title], { style: { color: dynamicStyles.h2.color }}) : null;
    ...
    const sections: { [key: string]: React.ReactNode } = {
        ...
        skills: (
            <section key="skills" className={styles.section}>
                ...
                {selectedTemplate === 'infographic' && styles.skillBar ? (
        ...
        return (
            <div className="flex h-full">
                <aside className={styles.sidebar} style={dynamicStyles.accentElement}>
                     {selectedTemplate !== 'creative' && selectedTemplate !== 'executive' && (
                    ...
                    {selectedTemplate !== 'creative' && <h1 className={styles.name} style={dynamicStyles.name}>{contact.name ? contact.name.toUpperCase() : 'YOUR NAME'}</h1>}
    ...
    return (
        <div id="resume-preview" className={`bg-white shadow-2xl ${styles.container} w-[8.5in] h-[11in]`} style={dynamicStyles.container}>
